...
export function main(salt, hash, pwdLen) {
	...
	// Each password char may be any ASCII value from 0x30 to 0x39
	// That's 10 possible values. A password of length N can have 10**N values.
	// Split into batches of HASHES_COUNT and get the count of that batches.
	const iterationsCount = Math.ceil(Math.pow(10, pwdLen) / HASHES_COUNT);
...
function getPassword(divmod_hi, divmod_lo, len) {
	...
	divmod_lo += divmod_hi % 10 * HASHES_COUNT;
	return (
		String.fromCharCode(divmod_lo % 10 + 48) +
		getPassword((divmod_hi / 10) >>> 0, (divmod_lo / 10) >>> 0, len - 1)
	);
}
...
void divmod(inout uvec2 state, out uint mod) {
	state[1] += state[0] % 10U * ${ HASHES_COUNT }U;
	mod = state[1] % 10U + 48U;
	state /= 10U;
}
